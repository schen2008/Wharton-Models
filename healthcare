#!/usr/bin/env python3
# industry_grouped_growth_tilted_fast_healthcare.py
#
# ✔ FAST
# ✔ Healthcare-only universe
# ✔ Handles missing data
# ✔ Skips bad tickers instantly
# ✔ Cache + --refresh
# ✔ PNG chart + Excel Top 20 (Healthcare only)
#
# Requirements:
...
# SETTINGS
# ------------------------------------------------------
CACHE_FILE   = "fundamentals_cache_healthcare.pkl"
SKIP_LOG     = "skipped_symbols_healthcare.txt"
OUT_EXCEL    = "growth_fast_healthcare_top20.xlsx"
CHART_FILE   = "healthcare_ranked_chart.png"
...
def export_full_chart(df):
    ...
    plt.title("Top 50 Healthcare Stocks (Score out of 100)")
    plt.ylabel("Score")
    plt.tight_layout()
...
def main():
    ...
    # --- 🔥 HEALTHCARE FILTER HERE 🔥 ---
    df = df[df["sector"] == "Healthcare"].copy()

    print(f"Healthcare count: {len(df)}")

    print("Scoring healthcare-only universe...")
    ranked = score(df)

    filtered = ranked[
        (ranked["marketCap"] >= MIN_MARKET_CAP) &
        (ranked["price"] >= MIN_PRICE) &
        (ranked["exchange_std"].isin(EXCHANGES_ALLOWED))
    ].copy()

    print("\n✅ Saving CSV files...")
    ranked.to_csv("healthcare_all_ranked.csv", index=False)
    ranked.head(20).to_csv("healthcare_top20.csv", index=False)
    print("Saved: healthcare_all_ranked.csv and healthcare_top20.csv")

    print("Exporting Excel...")
    with pd.ExcelWriter(OUT_EXCEL, engine="openpyxl") as writer:
        filtered.head(20).to_excel(writer, sheet_name="Healthcare", index=False)
